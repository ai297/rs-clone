...
test('getGame(id) should be null if game is not exists', () => {
  const target = new GameService(deckFactoryMock);
  expect(target.getGame('id')).toBeNull();
});

test('getGame(id) should be not null if game is exists', () => {
  const target = new GameService(deckFactoryMock);
  const id = '123';
  target.newGame(id);
  expect(target.getGame(id)).toBeTruthy();
});
